...
def createLayout():
    ...
    for i in range(3):
        ...
        for j in range(1, 4):
            ...
            disp.append(Text(background_color='#bbbbcc', size=(dimensions[0], dimensions[1]), key=f'{i * 3 + j}',
                             enable_events=True, font=("Helvetica", 25)))
        # add rows of display boxes
        display.append(disp)

    layout = [[Text(f"X:{X}\t\tO:{O}", key='score', font=('Cambria', 20))],
              [Text(text=" " * 23 + " Tic Tac Toe", enable_events=True, click_submits=True, key='click',
                    font=('Constantia', 12), text_color="#99CAFF")],
              *display,
              [Text(" " * 9),
               Text("X's turn", size=(20, 1), key='out', font=('Leelawadee', 12), background_color="#660066")],
              [Text()],
              [Text(" " * 5), Button(button_text='RESET BOARD', key='resetboard'),
               Button(button_text='RESET SCORE', key='resetscore')],
              [],
...
def checkWin():
    ...
    if hasWon == 0:
        ...
        if gameTable[1][1] != None:
            if (gameTable[0][0] == gameTable[1][1] and gameTable[1][1] == gameTable[2][2]) or (
                    gameTable[2][0] == gameTable[1][1] and gameTable[1][1] == gameTable[0][2]):
